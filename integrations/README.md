...
Codex model-catalog discovery is also separate from response routing. When the local account pool exposes plan and authentication status, catalog GETs use a stable valid OAuth account with the broadest plan visibility: Pro first, then Plus, then EDU/team/business/enterprise, then other valid OAuth plans. Accounts within a tier sort by account ID. This prevents a sticky EDU catalog session from removing models that a valid Pro account exposes. If local plan status is unavailable, the router keeps its existing OAuth catalog selection instead of guessing a tier. The upstream catalog remains authoritative; the integration does not add model names. Response sessions, manual new-chat selection and Automatic quota balancing remain unchanged.

Acceptance on 2026-09-19: the focused model-catalog tests passed, including valid-Pro priority, invalid-Pro fallback and stable same-tier selection. The Router binary built, was backed up and replaced, and the LaunchAgent restarted healthy and ready. A real client-version catalog GET returned the upstream `gpt-6-astra` entry. Router logs showed the synthetic catalog session move from the EDU account to the Pro account, while the existing manual new-chat policy remained on the same selected account. The acceptance sent no test response request.
...
## Weekly exhaustion and selector alignment — 2026-09-18

The native selector is constrained to the 296-point content width, including long account names. Its accessibility frame matched the title left edge; opening the menu and selecting the existing account succeeded. Release build and signature verification passed. Fixture assertions cover weekly exhaustion overriding 5h, unchanged raw 5h usage, and non-exhausted weekly quota preserving 5h availability. Installed UI switching from Weekly to 5h showed zero and the weekly reset/exhaustion explanation. Original display and account selection were restored. Pixel screenshot acceptance was unavailable; layout evidence is from accessibility geometry. No model requests or Automatic policy changes.

## Request-limit classification correction

The router can append a synthetic `Name=request-limit` record whose duration is seven days even for an unspecified request-time limit. The native view excludes that record from measured quota windows; its duration is not evidence of weekly exhaustion. Opening the panel triggers a refresh. This changes presentation only, not router exhaustion enforcement or account routing. The reset line always belongs to the selected window and precedes any weekly-exhaustion explanation.

Acceptance: scoped regression fixtures and release build passed; installed Pro click remained Weekly, Plus toggled between distinct weekly/5h reset captions and was restored. An exhausted weekly account showed the selected5h reset before its explanation. Account choice and router policy were preserved.
